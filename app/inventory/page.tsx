import { Suspense } from "react"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { prisma } from "@/lib/prisma"
import { InventoryActions } from "@/components/inventory/inventory-actions"
import { WarehouseSelector } from "@/components/inventory/warehouse-selector"
import { AddProductDialog } from "@/components/inventory/add-product-dialog"
import { TableSkeleton } from "@/components/ui/table-skeleton"

async function getWarehouses() {
  return await prisma.warehouse.findMany({
    orderBy: { name: "asc" },
  })
}

async function getInventoryByWarehouse(warehouseId?: number) {
  if (!warehouseId) return []

  return await prisma.warehouseProduct.findMany({
    where: { warehouseId },
    include: {
      product: true,
      warehouse: true,
    },
    orderBy: {
      product: { name: "asc" },
    },
  })
}

async function getAvailableProducts(warehouseId?: number) {
  if (!warehouseId) return []

  // Get products that are NOT in this warehouse
  const existingProductIds = await prisma.warehouseProduct.findMany({
    where: { warehouseId },
    select: { productId: true },
  })

  const existingIds = existingProductIds.map((item) => item.productId)

  return await prisma.product.findMany({
    where: {
      id: {
        notIn: existingIds,
      },
    },
    orderBy: { name: "asc" },
  })
}

interface InventoryPageProps {
  searchParams: { warehouse?: string }
}

async function InventoryContent({ searchParams }: InventoryPageProps) {
  const warehouses = await getWarehouses()
  const selectedWarehouseId = searchParams.warehouse ? Number.parseInt(searchParams.warehouse) : warehouses[0]?.id
  const inventory = await getInventoryByWarehouse(selectedWarehouseId)
  const availableProducts = await getAvailableProducts(selectedWarehouseId)
  const selectedWarehouse = warehouses.find((w) => w.id === selectedWarehouseId)

  const formatCurrency = (amount: number | null) => {
    if (!amount) return "-"
    return new Intl.NumberFormat("id-ID", {
      style: "currency",
      currency: "IDR",
      minimumFractionDigits: 0,
    }).format(amount)
  }

  return (
    <div className="container mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight">Inventaris Gudang</h1>
        <p className="text-muted-foreground">Kelola stok barang di setiap gudang</p>
      </div>

      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Stok Produk</CardTitle>
              <CardDescription>
                {selectedWarehouse
                  ? `Inventaris di ${selectedWarehouse.name}`
                  : "Pilih gudang untuk melihat inventaris"}
              </CardDescription>
            </div>
            <WarehouseSelector warehouses={warehouses} selectedId={selectedWarehouseId} />
          </div>
        </CardHeader>
        <CardContent>
          {!selectedWarehouse ? (
            <div className="text-center py-8 text-muted-foreground">Pilih gudang untuk melihat inventaris</div>
          ) : (
            <div className="space-y-4">
              {/* Add Product Button */}
              {availableProducts.length > 0 && (
                <div className="flex justify-end">
                  <AddProductDialog
                    warehouseId={selectedWarehouseId}
                    availableProducts={availableProducts}
                    onSuccess={() => window.location.reload()}
                  />
                </div>
              )}

              {inventory.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  Belum ada produk di gudang ini
                  {availableProducts.length > 0 && (
                    <div className="mt-4">
                      <AddProductDialog
                        warehouseId={selectedWarehouseId}
                        availableProducts={availableProducts}
                        onSuccess={() => window.location.reload()}
                      />
                    </div>
                  )}
                </div>
              ) : (
                <div className="rounded-md border">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>Nama Produk</TableHead>
                        <TableHead>Kode Produk</TableHead>
                        <TableHead>Unit</TableHead>
                        <TableHead className="text-right">Stok</TableHead>
                        <TableHead className="text-right">Harga Beli</TableHead>
                        <TableHead className="text-right">Harga Jual</TableHead>
                        <TableHead className="text-right">Aksi</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {inventory.map((item) => (
                        <TableRow key={`${item.warehouseId}-${item.productId}`}>
                          <TableCell className="font-medium">{item.product.name}</TableCell>
                          <TableCell>{item.product.code}</TableCell>
                          <TableCell>{item.product.unit || "-"}</TableCell>
                          <TableCell className="text-right font-medium">{item.stok}</TableCell>
                          <TableCell className="text-right">{formatCurrency(item.hargaBeli)}</TableCell>
                          <TableCell className="text-right">{formatCurrency(item.hargaJual)}</TableCell>
                          <TableCell className="text-right">
                            <InventoryActions item={item} warehouses={warehouses} />
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                </div>
              )}
            </div>
          )}
        </CardContent>
      </Card>
    </div>
  )
}

export default function InventoryPage({ searchParams }: InventoryPageProps) {
  return (
    <Suspense
      fallback={
        <div className="container mx-auto p-6">
          <div className="mb-6">
            <h1 className="text-3xl font-bold tracking-tight">Inventaris Gudang</h1>
            <p className="text-muted-foreground">Kelola stok barang di setiap gudang</p>
          </div>
          <Card>
            <CardHeader>
              <CardTitle>Stok Produk</CardTitle>
              <CardDescription>Memuat data inventaris...</CardDescription>
            </CardHeader>
            <CardContent>
              <TableSkeleton columns={7} rows={5} />
            </CardContent>
          </Card>
        </div>
      }
    >
      <InventoryContent searchParams={searchParams} />
    </Suspense>
  )
}
